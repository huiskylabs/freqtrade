# 30m/1h entry strategy: even faster timeframes with moderate risk
...
class TrendPullback30m1hModerate(PortfolioHeatMixin, TrendPullbackScaleOutStrategy):
    """
    30m entry timeframe + 1h bias (much faster than 2h/4h)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                dataframe["bias_long_1h"].fillna(False)
        ...
        dataframe.loc[
            (
                dataframe["bias_short_1h"].fillna(False)
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """Use 1h bias for exits"""
        dataframe["exit_long"] = 0
        dataframe["exit_short"] = 0
        dataframe.loc[dataframe["bias_short_1h"].fillna(False), "exit_long"] = 1
        dataframe.loc[dataframe["bias_long_1h"].fillna(False), "exit_short"] = 1
        return dataframe
